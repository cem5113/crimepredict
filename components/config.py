...
# GH token: secrets yoksa ortam değişkeninden al
GH_TOKEN    = st.secrets.get("GH_TOKEN", None) or os.getenv("GITHUB_TOKEN")

APP_NAME    = st.secrets.get("APP_NAME", "SUTAM – Suç Tahmin Modeli")
APP_ROLE    = st.secrets.get("APP_ROLE", "Kullanıcı")

ASSET_ZIP_1 = st.secrets.get("ASSET_ZIP_1", "fr-crime-pipeline-output")
